...
- [`10-auditoria-2026-09-07.md`](./02-arquitetura/10-auditoria-2026-09-07.md) — auditoria da queixa "para de transcrever após dias no ar" (processo vivo com 9,5 dias, log unificado, histórico, `coreaudiod`, reprodutores em `tools/diag/`) + revisão geral de código, build, release e testes. Causa-raiz **não confirmada** por falta de log persistido; os caminhos silenciosos do código (buffer curto descartado, transcrição vazia) são os candidatos. Propõe **Fase 5 — robustez em sessão longa**: diagnóstico persistido + self-healing (áudio, hotkey, decoder) + correção de bugs confirmados (stream de nível morto após a 1ª gravação, snapshots de permissão divididos entre dois consumidores, swap de modelo não persistido, appcast publicado antes do DMG, 37 chaves de localização ausentes).
- [`11-modulos-fase5.md`](./02-arquitetura/11-modulos-fase5.md) — snapshot **em execução** da Fase 5 (robustez em sessão longa). Tarefa 1 fechada: `Diag` + `DiagnosticsLog` (arquivo rotativo em `~/Library/Logs/Tagarela/`), marcos do pipeline promovidos a `.notice`/`.error` persistidos, e a linha que logava o ditado inteiro (`transcribed: '…'`) substituída por `transcribed chars=N` com teste de privacidade que varre os fontes. Tarefa 2: `PipelineHealth` (contadores de gravações/vazios/curtos/recuperações + uptime) na linha de saúde do menu, botão **Exportar diagnóstico** em Preferências › Sobre, e correção dos sub-labels mentirosos do menu (§5.3 da auditoria). Suíte 186 → 203.
...
- [`troubleshooting-runtime.md`](./03-funcionalidades/troubleshooting-runtime.md) — sintomas observados em uso real (v1.0.x) com remediação. Entrada inicial: `transcribed: ''` após horas + churn de Accessibility, resolvido com `Cmd+Q` total + relaunch — **revisada em 2026-09-07** (pico de áudio reinterpretado como ruído amplificado; roteiro de captura de evidência para a próxima ocorrência).
...
- [`ADR-0007-sparkle-version-vs-build-number.md`](./04-decisoes/ADR-0007-sparkle-version-vs-build-number.md) — `sparkle:version` no appcast deve ser o `CFBundleVersion` (build number), não a versão de marketing. Bug que quebrava detecção de update pra quem estava em build ≥ 2; o aceite da Fase 3 deu falso positivo por só testar v1.0.0→v1.0.1. Corrigido em `appcast.sh` + `appcast.xml`.
