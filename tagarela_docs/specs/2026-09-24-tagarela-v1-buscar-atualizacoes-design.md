---
data: 2026-09-24
status: implementado, aceito e publicado na v1.0.6 (2026-09-24)
...
**Resultado (2026-09-24):** o usuário clicou no item e o Sparkle respondeu *"Tagarela 1.0.6-dev é a versão mais recente disponível."* — build 6 contra os 6 publicados, a resposta certa. O nome mostrado é o `CFBundleShortVersionString` do build de aceite; a comparação do Sparkle é pelo `CFBundleVersion` (ADR-0007). O último item foi fechado depois da publicação — ver o fim deste documento.
...
- [x] Depois de publicada a v1.0.6, o mesmo item a oferece — e a atualização instala com as permissões intactas. **Com ressalva:** o Microfone foi pedido de novo — por causa do host de testes, não da atualização (ver o fim deste documento).
...
**Último item do aceite — ok, com ressalva (2026-09-24, 23:20).** No build de
aceite `1.0.6-dev` (build 6, Developer ID, instalado em `/Applications` às 23:07),
o usuário clicou em "Buscar atualizações…"; o Sparkle ofereceu a 1.0.6 (build 7),
instalou e reabriu, e o ditado cola. Monitoramento de Entrada e Acessibilidade vieram
concedidos sem pedido nenhum — no log do TCC, `ListenEvent`, `Accessibility` e
`PostEvent` com `authValue=2` já no primeiro launch. O **Microfone foi pedido de
novo**, e não por causa da atualização: o host de testes do `xcodebuild test`,
rodado às 23:09 para esta release, tinha tomado a concessão do microfone. Evidência
e mecanismo em *Reincidência em 2026-09-24* no
[`cleanup-fase3.md`](../04-decisoes/cleanup-fase3.md).
