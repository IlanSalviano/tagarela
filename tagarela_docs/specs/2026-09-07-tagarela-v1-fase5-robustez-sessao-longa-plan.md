...
- [ ] **Step 7 — build + aceite manual em build local:** Bloco A do checklist (§Tarefa 12). Commit: `fix(audio): engine por gravação + watchdog + stats + níveis por gravação; AudioMath extraído`.
  - Código commitado e suíte verde (203 → **215**). `test_levels_arrive_in_second_recording` esteve **vermelho** antes da correção (falhava na 2ª gravação) e ficou verde depois. **Aceite manual do Bloco A pendente com o usuário** — roteiro em [`fase5-manual.md`](../03-funcionalidades/checklists/fase5-manual.md).
